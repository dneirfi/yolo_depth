...
Camera::Camera(const char* dev_name, struct camera_format* conf, const char* format_string, const unsigned char disable_libv4l2) :
    dev_name(dev_name), disable_libv4l2(/*1*/ disable_libv4l2)
{
    /* open device */
...
void Camera::get_configurations(std::vector<std::string>& formats, std::vector<std::string>& controls)
{
	...
	for (/*auto*/std::map<std::string, v4l2_frmivalenum>::iterator it = valid_ratio_list.begin(); it != valid_ratio_list.end(); ++it) {
	...
	for (/*auto*/std::map<std::string, v4l2_queryctrl>::iterator it = valid_control_list.begin(); it != valid_control_list.end(); ++it) {
