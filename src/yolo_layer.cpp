...
void forward_yolo_layer(const layer l, network net)
{
    ...
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
            ...
            activate_array(l.output + index, (1+l.klasses)*l.w*l.h, LOGISTIC);
        }
    }
#endif
    ...
    for (b = 0; b < l.batch; ++b) {
            ...
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    ...
                    if (best_iou > l.truth_thresh) {
                        ...
                        int klass = net.truth[best_t*(4 + 1) + b*l.truths + 4];
                        if (l.map) klass = l.map[klass];
                        int class_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 4 + 1);
                        delta_yolo_class(l.output, l.delta, class_index, klass, l.classes, l.w*l.h, 0);
        ...
        for(t = 0; t < l.max_boxes; ++t){
            ...
            if(mask_n >= 0){
                ...
                int klass = net.truth[t*(4 + 1) + b*l.truths + 4];
                if (l.map) klass = l.map[klass];
                int class_index = entry_index(l, b, mask_n*l.w*l.h + j*l.w + i, 4 + 1);
                delta_yolo_class(l.output, l.delta, class_index, klass, l.classes, l.w*l.h, &avg_cat);
