...
void validate_classifier_10(char *datacfg, char *filename, char *weightfile)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        free(pred);
        if(indexes[0] == klass) avg_acc += 1;
        for(j = 0; j < topk; ++j){
            if(indexes[j] == klass) avg_topk += 1;
        }
...
void threat_classifier(char *datacfg, char *cfgfile, char *weightfile, int cam_index, const char *filename)
{
    ...
    while(1){
        ...
        image in = get_image_from_stream_ocv(cap, 0 /*full camera image*/, 0 /*no depth estimatiion*/, false /*dummy for gray_4_stereo*/, NULL, NULL);
        if(!in.data) break;
...
void gun_classifier(char *datacfg, char *cfgfile, char *weightfile, int cam_index, const char *filename)
{
    ...
    while(1){
        ...
        //image in = get_image_from_stream(cap);
        image in = get_image_from_stream_ocv(cap, 0 /*full camera image*/, 0 /*no depth estimation*/, false /*dummy for gray_4_stereo*/, NULL, NULL);
        image in_s = resize_image(in, net->w, net->h);
